...
#'
#' @examples
lira_rnaseq_pipline <- function(pat_id, eset = NULL, data_type = "tpm", palette = "nrc", path1 = NULL, path2 = NULL){
  ...
  ###################################
  if(is.null(eset)){
    eset <- get_lira_data(id = pat_id, data_type = data_type)  #也可选择 tpm_scale
    print(eset[1:5, 1:5])
  }
  ...
  p<- p1|p2
  ggsave(p, filename = paste0("0-score-and-BOR.pdf"), width = 12, height = 6.3, path = file_name$folder_name)
  #####################################
  ...
  sig_surv_plot(input_pdata =  input, 
                ...
                index       = pat_id, 
                project     = paste0("OS-",round(mean(input$LIRA), 2)))
  #####################################
  ...
  target <- 'IFNG_signature_Ayers_et_al.x'
  p<- sig_box(data = input, signature = target, variable = "BOR")
  
  ggsave(p, filename = paste0(target,"-and-BOR.pdf"), width = 7, height = 6.3, path = file_name$folder_name)
  ######################################################
  bc1 <- best_cutoff2(pdata = input, variable = "IFNG_signature_Ayers_et_al.x", time = "PFS_months", status = "PFS_status" )
  lira_score_location(score        = input, 
                      pat_id       = pat_id,
                      col_score    = "IFNG_signature_Ayers_et_al.x",
                      title        = "GEP",
                      ref_score    = NULL,
